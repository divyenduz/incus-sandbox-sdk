...
async function startImpl(this: CliContext, flags: {}, name: string): Promise<void> {
  try {
    const sandbox = await incus.sandbox.getByName(name);
    await sandbox.start();
    this.process.stdout.write(`Started ${name}\n`);
  } catch (err) {
